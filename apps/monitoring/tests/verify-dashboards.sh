...
while IFS= read -r file; do
  ...
  jq -e '
    [
      ...
      | select((has("datasource") | not) or (.datasource == null) or (.datasource == ""))
    ] | length == 0
  ' "$file" >/dev/null || fail "dashboard query target is missing an explicit datasource: ${rel}"

  jq -e '
    [
      .. | strings
      | select(test("\\$\\{DS_[A-Z0-9_]+\\}"))
    ] | length == 0
  ' "$file" >/dev/null || fail "dashboard contains unresolved imported datasource placeholders: ${rel}"

  jq -e '
    [
      .templating.list[]?
      | select(.type == "datasource")
      | select((.query == "prometheus") and ((.current.value // "") == "mimir"))
      | select(((.regex // "") != "") and ("Mimir" | test(.regex) | not))
    ] | length == 0
  ' "$file" >/dev/null || fail "dashboard datasource variable regex excludes Mimir: ${rel}"

  jq -e '
    [
      .templating.list[]?
      | select(.name | test("(?i)^(instance|instances|pod|container|controller)$"))
      | select((.multi != true) or (.includeAll != true) or ((.allValue // "") != ".*"))
    ] | length == 0
  ' "$file" >/dev/null || fail "per-instance dashboard variables must support multi-select All=~.*: ${rel}"

  jq -e '
    [
      .. | objects | select(has("expr")) | .expr
      | select((type == "string") and test("(instance|pod|container|controller)=\"\\$"))
    ] | length == 0
  ' "$file" >/dev/null || fail "dashboard uses exact matcher for multi-select instance-like variable: ${rel}"
  ...
  if [ "$rel" = "cnpg/cnpg-backup.json" ]; then
    ...
    jq -e '
      [
        .. | objects | select(has("expr")) | .expr
        | select(test("controller_runtime_reconcile_total") and (test("increase\\(") | not))
      ] | length == 0
    ' "$file" >/dev/null || fail "CNPG backup dashboard must show recent reconcile errors, not raw cumulative counters"
  fi

  if [ "$rel" = "traefik/traefik.json" ]; then
    jq -e '
      [
        .. | objects | select(has("expr")) | .expr
        | select(test("label_replace\\(") and test("\"service\"[[:space:]]*,[[:space:]]*\"\\$1\"[[:space:]]*,[[:space:]]*\"service\""))
      ] | length == 0
    ' "$file" >/dev/null || fail "Traefik dashboard must not overwrite service labels with label_replace"
  fi
